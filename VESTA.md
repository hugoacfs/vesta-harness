...
| `packages/vesta/voice/` | `@deepseek-ai/dsh-vesta-voice` | (Phase A) host voice bridge: LiveKit token route, room ↔ session binding, spoken-mode prompt section |
| `packages/client/ui-vesta-voice/` | `@deepseek-ai/dsh-client-ui-vesta-voice` | (Phase A) mic button + call HUD |
...
- **D1 Bridge topology.** The proven Python LiveKit agent stays; the DSH host plugin dials out to it over loopback (`ws://127.0.0.1:8490`). DSH stays bound to `127.0.0.1`; nothing on `0.0.0.0`. An all-Node `@livekit/agents` in-process agent is a later option, not now.
