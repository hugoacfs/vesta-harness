...
The call bar also carries a speech-speed chip (1.0× to 1.3×): the choice is posted to `POST /api/vesta/voice/config` for the bound call, applied by the agent from its next utterance, and remembered in `localStorage` (`vesta.voice.speed`) so the next call starts at that pace. Filler clips keep the speed they were rendered at.
## Summary
