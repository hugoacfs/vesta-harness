 ...
 * Unprefixed: the host matches proxy-stripped requests against it; callers
 * that hand the URL to the browser prepend MOUNT_PREFIX themselves.
 */
...
function prepareSource(resource: ComboResource): PreparedSource {
  ...
  const fallbackSource = sourceUrl === undefined
    ? `${MOUNT_PREFIX}/plugins/${resource.id}/${resource.fileName}`
...
export class ClientModuleRegistry extends Service {
  ...
  private chunkResponse(requestUrl: URL): LazyResponse | undefined {
    ...
    // Client-facing: the browser resolves `sourceMappingURL` against the origin,
    // so the reverse-proxy prefix must be carried (empty at the site root).
    const sourceMapUrl = `${MOUNT_PREFIX}${chunkUrl(record.entry.id, fileName, record.entry.rev, true)}`
  ...
  private async bundleResource(method: string | undefined, url: string): Promise<{
    ...
    // Package-local chunks are matched on the stripped request (chunkUrl is
    // unprefixed on purpose) and cached under that key.
