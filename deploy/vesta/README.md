...
`$DSH_HOME/cordis.patch.yml` is the machine-wide patch layer (every profile). **The Loader watches this file: an edit on a running harness re-applies the plugin tree at once and aborts every running turn** (production, 2026-09-12: a search-index row appended at 13:47 cut a user turn mid-compaction). Edit it only right before the restart that follows, or stop the unit first. It carries the host-plane MCP mounts that depend on this box — today only the Telegram notifier (`ai-telegram-mcp`, loopback 7335 → `mcp__telegram-notify__notify`). Memory and search are mounted by the versioned `dsh-vesta-app` bundle (`mcp__memory__*`, `mcp__search__*`); do not add them here again: a second `vesta-search` row (2026-09-09..11) put every search tool twice in the model's catalogue. The default agent preset is `vesta-default` (lean single agent: no delegation, no DeepSeek web search, web lookups through the search MCP); `vesta-orch` stays for fan-out and `vesta-voice` for calls (`settings.yaml` → `agent-presets.default`).
...
**Programmatic tool calling (roadmap T12, measured 2026-09-17; off by default).** Upstream's PTC lets a preset present its tools as a `run_code` transport plus a generated TypeScript SDK (`dsh-agent-tool-presentation` row with `mode: ptc` or `both`; the program runs in the host row `code-runtime`, `dsh-code-runtime-worker-thread`). What is wired: the presentation package is a bundle dependency; the `code-runtime` row carries budgets (busy time 120 s, wall 600 s, output 1 MiB, heap 512 MB); `vesta-modes` refuses `run_code` below the workspace-write tier on the `tools/pre-execute` waterfall (`ptc.guard`, `ptc.tiers`), because model code in the worker runs with the harness's own authority, outside the bash sandbox, while the tool bindings it calls are still policed; the `vesta-build` preset carries the row commented out with the numbers. Measured on staging (Build, Qwen lane, 20 Home Assistant tools connected, five read-only tasks in the harness checkout; `deploy/vesta/verify/t12-bench.sh <label>`): first-request input tokens native 24.2 k, `ptc` 24.0 k, `both` 41.0 k — the generated SDK is about as large as the native schemas, so `both` pays twice. Steps per task native 4/3/5/8/2 against `ptc` 3/4/2/3/2 (fewer on three of five), total input tokens lower on three of five under `ptc` (the TODO search fell from 192 k to 75 k), wall time higher on four of five (the model writes a program instead of one short call); all ten `run_code` calls under `ptc` succeeded, so Qwen's tool-call parser handled long code arguments. Under `ptc` alone a direct tool call is refused by upstream, so a `ptc` session switched to a read-only mode with `/mode` would have no usable tool at all — enable `ptc` only in presets that never switch down, or refuse the switch. `t12-verify.sh` checks the header (SDK section, `run_code`), the guard's refusal in a read-only session and after `/mode research`. Decision on enabling `ptc` in Ops or Build is Hugo's; until then every mode is native.
