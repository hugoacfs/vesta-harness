...
- `ffdrive/voice-connect-check.mjs` (2026-09-25, `BROWSER=chrome`) — the vesta-voice page in Chrome with a fake microphone: a real call through the path connects, a call after a page refresh mid-call connects, a call whose offer is forced to fail returns the page to idle with a note. Headless Firefox on the Mac cannot complete ICE against the box, so use Chrome.
