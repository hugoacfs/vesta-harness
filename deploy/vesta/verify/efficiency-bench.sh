#!/bin/bash
# Efficiency benchmark on staging (engine-fit phase): five batch-shaped and three interactive tasks in a given preset.
# Usage: efficiency-bench.sh <preset> <label> [warm-only]
...
set -u
PRESET=${1:-vesta-ops}; LABEL=${2:-$PRESET}; WARM_ONLY=${3:-}
...
echo "== efficiency bench: $LABEL / $PRESET ($(date +%H:%M))"
run_task warm "Reply with the single word OK and nothing else."
if [ "$WARM_ONLY" = "warm-only" ]; then echo "== done $LABEL (warm only)"; exit 0; fi
...
run_task quick "What is the local time on this server right now? One line, no tools needed."
run_task explain "In three lines, what does the vesta-modes host plugin do? Read packages/vesta/vesta-modes/README.md or its source first. Do not modify anything."
run_task lookup "Is jar-prod.sh mentioned in deploy/vesta/verify/README.md? Answer yes or no with the line number. Do not modify anything."
