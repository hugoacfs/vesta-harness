...
# usage: voice-echo-scenarios.sh <label> <scenarios...>   scenarios: s1 s1b s2n s2 s3 s5 s4 s6 s7 s7b s8
...
docker exec $C test -f /tmp/pause_long.wav || { docker exec $C python /tmp/say.py "two hundred and seventy pounds a month, and it should be petrol, an estate or a small SUV, with low mileage, and I don't mind whether it is manual or automatic." /tmp/half2long.wav; docker exec $C python /tmp/mkpause.py /tmp/half1.wav /tmp/half2long.wav /tmp/pause_long.wav 4.5 0.3; }
