...
- Verify (synthetic caller, `deploy/vesta/bin/vesta-call-check` run inside the agent container: joins the room as a STANDARD participant, plays a Kyutai-synthesized question, reports `lk.agent.state` transitions and audio received back). ⏳
