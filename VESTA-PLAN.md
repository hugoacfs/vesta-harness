...
| B | Autonomy and approvals by voice | in progress (written 2026-09-05, deploying) |
...
- Verify: scripted three-turn call from the agent container ("switch to safe mode" → confirmation; "create a file…" in `read-only` → spoken approval question + on-screen card; "yes" → "Okay, going ahead.", card closes, continuation spoken), plus the session log showing `permission/preset`, `approval/asked`, `approval/decided`.
